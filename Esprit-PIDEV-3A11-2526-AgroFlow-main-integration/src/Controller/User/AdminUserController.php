...
class AdminUserController extends AbstractController
{
 ...
 // ── CREATE ────────────────────────────────────────────────────────────────
    ...
    // ── BAN ───────────────────────────────────────────────────────────────────
    #[Route('/{cin}/ban', name: '_ban', requirements: ['cin' => '\d+'], methods: ['POST'])]
    public function ban(Request $request, User $user): Response
    {
        // Sécurité : on ne peut bannir qu'un agriculteur
        if ((int)$user->getRole() !== 2) {
            throw $this->createAccessDeniedException('Seuls les agriculteurs peuvent être bannis.');
        }

        if ($this->isCsrfTokenValid('ban_user_' . $user->getCin(), $request->request->get('_token'))) {
            $user->setRole(0);
            $user->setDateDernierchg(new \DateTime());
            $this->em->flush();
            $this->addFlash('success', 'Le compte de ' . $user->getPrenom() . ' ' . $user->getNom() . ' a été banni.');
        }
    ...
    // ── UNBAN ─────────────────────────────────────────────────────────────────
    #[Route('/{cin}/unban', name: '_unban', requirements: ['cin' => '\d+'], methods: ['POST'])]
    public function unban(Request $request, User $user): Response
    {
        // Sécurité : on ne peut débannir qu'un compte banni
        if ((int)$user->getRole() !== 0) {
            throw $this->createAccessDeniedException('Ce compte n\'est pas banni.');
        }

        if ($this->isCsrfTokenValid('unban_user_' . $user->getCin(), $request->request->get('_token'))) {
            $user->setRole(2); // On remet agriculteur
            $user->setDateDernierchg(new \DateTime());
            $this->em->flush();
 ...
 // ── DELETE ────────────────────────────────────────────────────────────────
    ...
    public function pdf(User $user): Response
    {
        ...
        <body>
            ...
            <div class="content">
                ...
                <div class="row"><span class="label">Statut</span><span class="value">' . $roleLabel . '</span></div>
    ...
    public function excel(): StreamedResponse
    {
        ...
        $sheet->setTitle('Agriculteurs');

        $headers = ['CIN', 'Nom', 'Prénom', 'Email', 'Téléphone', 'Ville', 'Adresse', 'Statut', 'Date inscription', 'Dernière modif'];
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="agriculteurs-' . date('Y-m-d') . '.xlsx"');

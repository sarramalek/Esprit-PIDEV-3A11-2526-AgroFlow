...
Materials Module  -- Eya Laadjimi 

Automatically generated interventions: automatic generation of interventions based on detected failures, equipment 
condition, or scheduled maintenance dates, in order to improve organisation and reduce manual interventions.

AI-generated recommendations: use of artificial intelligence to analyse maintenance data and suggest tailored solutions, preventive advice, and corrective actions.

Automatic reminder calendar (Google Calendar): automatic sending of reminders for periodic or annual maintenance tasks to avoid oversights and ensure compliance with service dates.

Suggestions based on maintenance history (Gemini): analysis of the complete maintenance history to identify recurring failures, suggest improvements, and facilitate decision-making.

Maintenance workshop locator (OpenStreetMap): display of maintenance workshops and shops on an interactive map to identify their location and facilitate access to maintenance centres.
...
Geolocation with OpenStreetMap + Gemini AI – Interactive map powered by Leaflet.js and OpenStreetMap that displays partners and veterinarians on the map; 

integrated with Google Gemini to provide intelligent location context and recommendations Nearest Veterinarian / Partner Finder – Automatically detects the user's current position and calculates the closest available veterinarian or partner using GPS distance algorithms Animal 

Food Suggestion (Open Food Facts API) – Integration with the Open Food Facts API to suggest appropriate and nutritionally validated food products for different animal species 

Statistics Dashboard – Visual analytics covering: Distribution and count of fragile/endangered species under care Cancelled examination statistics (monthly and annual views) Monthly and annual trend charts for health monitoring Vaccination Alert System 

– Automated reminders and alerts for upcoming, due, and overdue animal vaccinations, with per-animal scheduling Text-to-Speech (Application-wide) 

– Full TTS support integrated throughout the module, enabling audio readback of partner info, animal records, alerts, and recommendations for accessibility Multi-language Translation 

– Built-in language translation support across the entire application interface, allowing users to switch between languages dynamically Animal & Species Encyclopedia (Wikipedia API) 

– Browse a rich encyclopedia of animal breeds and species powered by the Wikipedia API, with detailed descriptions, taxonomic information, and care guides Pagination

 – Smart pagination across all listing views (partners, animals, examinations, vaccinations) for efficient data navigation Best Breeding Pair Suggestion (Accouplement Algorithm) 
 
 – An intelligent matching algorithm that analyzes genetic compatibility, health records, age, and species criteria to recommend the optimal breeding pairs, maximizing offspring health and genetic diversity

 Stocks Module -- Eya Mallouli 

QR Server API — Used as the engine for fast QR Code generation.

Symfony Mailer — For sending automatic alert emails.

Telegram Bot API — Used for sending real-time alert notifications on mobile.

ExchangeRate-API — Used for automatic currency conversion to TND.

Google Translate API (GTX) — Used for real-time translation of product descriptions into English, Arabic, and French.

Google Gemini API (1.5 Flash) — For Smart Vision (automatic product identification via photo scan).

Nominatim API (OpenStreetMap) — Used for geolocation (finding the nearest suppliers in case of stock shortage).
...
-Eya Laadjimi - Materiels Module 
- Yessmine Rezgui - Animals HealthCare Module
- Eya Mallouli - Stocks Module

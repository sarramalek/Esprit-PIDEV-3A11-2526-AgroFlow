...
class EmailService
{
    ...
    public function envoyerMailAlerte(Article $article, ?User $destinataire = null): bool
    {
        ...
        $html = '<div style="font-family: Arial, sans-serif; border: 1px solid #eee; padding: 20px; border-radius: 8px;">'
            ...
            . '<ul>'
            . '<li>Stock actuel : <strong>' . $article->getQuantiteEnStock() . '</strong> ' . htmlspecialchars($article->getUniteMesure() ?? 'unités') . '</li>'
            . '<li>Seuil d\'alerte : <strong>' . $article->getSeuilAlerte() . '</strong> ' . htmlspecialchars($article->getUniteMesure() ?? 'unités') . '</li>'
            . '</ul>'
